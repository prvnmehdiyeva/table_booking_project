...
import { CodeloungeService } from './service/codelounge.service';

@Component({
  selector: 'app-codelounge',
  templateUrl: './codelounge.component.html',
  styleUrl: './codelounge.component.scss'
})
export class CodeloungeComponent implements OnInit {
  table1Seats:any[]=[]
  table1Id:any[]=[]
  roomName!:string
  roomDes!:string
  ...
  constructor(
    ...
    private codeSrv:CodeloungeService
  )
  {}
  ...
  fetchSeats(){
   ...
   availableSeats(){
    this.srv.getBookings().subscribe((data) => {
      ...
      data.forEach((booked: any) => {
        if (booked.roomNumber === "2") {
         room1BookingCount++; 
         console.log(this.rowLength);
         console.log(room1BookingCount);
         this.available = this.rowLength * 4 - room1BookingCount
        }
      });
    
      console.log("Total bookings for room 2:", room1BookingCount);
    });
   ...
   addRow(){
    ...
    for(let i = 0; i < numToAdd; i++) {
        ...
        this.codeSrv.addRow(newRow).subscribe(() => {
            this.fetchSeats();
            this.numToAdd = 0
        });
    }
...
delRow(){
  ...
  for(let i = 0; i < numToDelete; i++) {
      ...
      this.codeSrv.delRow(rowIdToDelete).subscribe(() => {
          this.fetchSeats();
          this.numToDelete = 0
      });
  }
